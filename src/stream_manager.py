...
class StreamManager:
    ...
    async def proxy_hls_segment(
        ...
        # For pre-built VOD manifests, wait for the segment to be transcoded
        # BEFORE creating the StreamingResponse.  This prevents the HTTP 200
        # from being sent while the body is still empty — hls.js would abort
        # after ~20 s of no data (fragLoadingTimeOut).
        resolved_segment_path: Optional[str] = None
        if segment_url.startswith("file://") and self.pooled_manager:
            path = segment_url[7:]
            if not os.path.exists(path):
                segment_name = os.path.basename(path)
                shared_process = self.pooled_manager.get_process_by_stream_id(stream_id)
                if shared_process and shared_process.prebuilt_manifest:
                    logger.info(
                        f"Segment {segment_name} not yet on disk for stream {stream_id}, "
                        f"waiting (highest_segment={shared_process.highest_segment})"
                    )

                    # Heartbeat keeps client/stream alive while we
                    # wait for ffmpeg to produce the segment
                    def _heartbeat():
                        now = datetime.now(timezone.utc)
                        if client_id in self.clients:
                            self.clients[client_id].last_access = now
                            self.clients[client_id].last_data_time = now
                        if stream_id in self.streams:
                            self.streams[stream_id].last_access = now

                    resolved = await shared_process.wait_for_segment(
                        segment_name, heartbeat_callback=_heartbeat
                    )
                    if resolved:
                        resolved_segment_path = resolved
                        logger.info(
                            f"Segment {segment_name} ready for stream {stream_id}"
                        )
                    else:
                        logger.warning(
                            f"Segment {segment_name} not available after waiting for stream {stream_id}"
                        )
                        return Response(
                            content=b"Segment not yet available",
                            status_code=503,
        ...
        async def segment_generator():
            ...
            while retry_count <= max_retries:
                try:
                    ...
                    # For pre-built VOD manifests the file:// URL may reference a segment that
                    # hasn't been transcoded yet — but if we have a resolved_segment_path from
                    # the pre-wait above, the file is guaranteed to be on disk already.
                    if segment_url.startswith("file://") or os.path.exists(segment_url):
                        # Use the pre-resolved path if available, otherwise strip file://
                        path = resolved_segment_path or (
                            segment_url[7:]
                            if segment_url.startswith("file://")
                            else segment_url
                        )

                        # Defensive check: the file could have been cleaned up
                        # between the pre-wait and the generator starting
                        if not os.path.exists(path):
                            raise FileNotFoundError(
                                f"Segment file disappeared: {path}"
                            )
